...
const matriculaController = {
  // Buscar matrículas por nome ou código de pessoa
  async buscarMatriculas(req, res) {
    try {
      ...
      if (!isNaN(busca)) {
        resultado = await Matricula.buscarPorPessoa(parseInt(busca), parseInt(page), parseInt(limit));
      } else {
        resultado = await Matricula.buscarPorNome(busca, parseInt(page), parseInt(limit));
      }
